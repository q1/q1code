/**
 * Pure state and labels for the Prism settings screen (the CLIProxyAPI
 * account pool on mobile): which environments show it, status and error
...
import { FORK_CONFIG_FILENAME, type CliProxyRoutingStrategy } from "@q1code/core/config";
import { envVarForFlag } from "@q1code/core/flags";
import { type CliProxyClientError, readForkFlag } from "@t3tools/client-runtime/fork";
...
export interface CliProxyEnvironmentRef {
  readonly environmentId: EnvironmentId;
  readonly label: string;
}

/** Connected environments whose `cliproxy` flag is on, in catalog order. */
export function selectCliProxyEnvironments(
...
): ReadonlyArray<CliProxyEnvironmentRef> {
  return environments
    .filter(
      (environment) =>
        environment.connection.phase === "connected" &&
        readForkFlag(capabilitiesOf(environment.environmentId), "cliproxy"),
    )
    .map((environment) => ({
...
export const CLIPROXY_OFF_HINT = `Prism is off. Set ${envVarForFlag("cliproxy")}=1 or flags.cliproxy in ${FORK_CONFIG_FILENAME} on the server, restart it, and this screen fills in.`;
...
export interface CliProxyStatusLine {
  readonly label: string;
 ...
 */
export function describeCliProxyStatus(
  status: CliProxyStatus,
  relative: (iso: string) => string,
): ReadonlyArray<CliProxyStatusLine> {
  const lines: Array<CliProxyStatusLine> = [
    { label: "Mode", value: status.mode === "external" ? "External" : "Sidecar" },
    { label: "Base URL", value: status.baseUrl ?? `port ${status.port}` },
  ];
  if (status.version) lines.push({ label: "Version", value: status.version });
...
export function isCliProxyPermissionError(error: CliProxyCallError): boolean {
  return (
...
export function describeCliProxyUnavailable(
  ...
  switch (reason) {
    case "flag-off":
      return CLIPROXY_OFF_HINT;
    case "sidecar-not-ready":
      return state === "failed"
        ? `CLIProxyAPI failed to start. Check the server log and the cliproxy section of ${FORK_CONFIG_FILENAME}.`
        : `CLIProxyAPI is ${CLIPROXY_STATE_LABELS[state].toLowerCase()}. Accounts appear once it is ready.`;
...
export function describeCliProxyError(error: CliProxyCallError): string {
  switch (error._tag) {
    case "CliProxyUnavailableError":
      return describeCliProxyUnavailable(error.reason, error.state);
...
export function describeCliProxyAccount(
  account: CliProxyAccount,
  relative: (iso: string) => string,
): string {
  const parts = [labelCliProxyProvider(account.provider)];
...
export const INITIAL_ACCOUNTS_STATE: CliProxyAccountsState = {
  accounts: null,
  error: null,
  pending: {},
  rowErrors: {},
...
export type CliProxyAccountsEvent =
  ...
  | { readonly type: "toggled"; readonly id: string; readonly account: CliProxyAccount }
...
const replaceAccount = (
  ...
  id: string,
  update: (account: CliProxyAccount) => CliProxyAccount,
...
export const CLIPROXY_LOGIN_PROVIDERS: ReadonlyArray<{
  readonly value: CliProxyLoginProvider;
  readonly label: string;
}> = [
...
export function labelCliProxyLoginProvider(provider: CliProxyLoginProvider): string {
  return CLIPROXY_LOGIN_PROVIDERS.find((entry) => entry.value === provider)?.label ?? provider;
}
...
export const CLIPROXY_ROUTING_OPTIONS: ReadonlyArray<{
  readonly value: CliProxyRoutingStrategy;
  readonly label: string;
}> = [
...
export const CLIPROXY_STATUS_POLL_MS = 10_000;
export const CLIPROXY_LOGIN_POLL_MS = 2_000;
export const CLIPROXY_RESTART_POLL_MS = 2_000;
export const CLIPROXY_RESTART_TIMEOUT_MS = 30_000;

/** Background status polling only costs a request while someone can see the answer. */
export function shouldPollCliProxyStatus(input: {
...
export function nextRestartStep(input: {
  ...
  return input.elapsedMs >= CLIPROXY_RESTART_TIMEOUT_MS ? "timeout" : "poll";
}

// Settings row

export type CliProxyOverview =
 ...
 */
export function summarizeCliProxyOverviews(
  overviews: ReadonlyArray<CliProxyOverview>,
): string | undefined {
  if (overviews.length === 0 || overviews.some((overview) => overview._tag === "loading")) {
    return undefined;
  }
  const loaded = overviews.filter(
    (overview): overview is Extract<CliProxyOverview, { _tag: "loaded" }> =>
  ...
  if (notReady) return CLIPROXY_STATE_LABELS[notReady.state];
