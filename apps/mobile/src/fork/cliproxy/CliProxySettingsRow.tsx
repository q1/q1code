 ...
 * screen, shown only while a connected environment has the `cliproxy` flag
...
import {
  type CliProxyOverview,
  selectCliProxyEnvironments,
  summarizeCliProxyOverviews,
} from "./cliproxySettings.logic";
import { useCliProxyApi } from "./useCliProxyApi";

export function CliProxySettingsRow() {
  ...
  const [overviews, setOverviews] = useState<ReadonlyMap<EnvironmentId, CliProxyOverview>>(
    () => new Map(),
  );
  const handleOverview = useCallback((environmentId: EnvironmentId, overview: CliProxyOverview) => {
    setOverviews((previous) => new Map(previous).set(environmentId, overview));
  }, []);

  if (targets.length === 0) return null;

  const value = summarizeCliProxyOverviews(
...
function OverviewLoader(props: {
  ...
  readonly onOverview: (environmentId: EnvironmentId, overview: CliProxyOverview) => void;
}) {
  const api = useCliProxyApi(props.environmentId);
