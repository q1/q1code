  ...
  providers: [
    {
      ...
      models: [{ slug: "claude-test", name: "Claude test", capabilities: null }],
    },
  ],
...
const observation = (available: boolean) => ({
  value: {
    serviceInstanceId: "pc",
    pairingRevision: 1,
...
describe("Prism coding model eligibility", () => {
  ...
  it("keeps an unavailable draft selected while disabling its picker row", () => {
    const options = applyMicPrismModelAvailability(
      config,
      buildModelOptions(config, selection),
      selection,
      observation(false),
    );
  ...
  it("disables stale observations even when the last response was eligible", () => {
    const options = applyMicPrismModelAvailability(
      ...
      buildModelOptions(config, selection),
      selection,
      { ...observation(true), error: "Prism offline" },
    );
    expect(options[0]?.isUnavailable).toBe(true);
  ...
  it("preserves the explicit direct route when choosing another model", () => {
    const direct = {
      ...selection,
      model: "older-model",
      options: [{ id: "prism-route", value: "direct" }],
    };
    const options = applyMicPrismModelAvailability(
      config,
      buildModelOptions(config, direct),
      direct,
      observation(false),
    );
