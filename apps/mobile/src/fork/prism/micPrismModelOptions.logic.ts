...
/** Local provider sign-out cannot switch an explicitly selected pooled draft. */
export function resolveMicPrismSelectableModelSelection(
  config: ServerConfig | null | undefined,
  selection: ModelSelection | null,
) {
  return isMicPrismModel(config, selection)
    ? selection
    : resolveSelectableModelSelection(config, selection);
}
