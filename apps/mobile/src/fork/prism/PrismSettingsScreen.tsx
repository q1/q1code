 ...
 * environment with the `prism` flag on. Per environment: proxy status with
...
export function PrismSettingsScreen() {
...
export const prismSettingsStackScreen = createNativeStackScreen({
  screen: PrismSettingsScreen,
  linking: "prism",
  options: {
    title: "Prism",
  },
...
function EnvironmentPanel(props: {
  ...
  const loadAccounts = useCallback(async () => {
    ...
    dispatchAccounts(
      result._tag === "ok"
        ...
        : { type: "loadFailed", error: describePrismError(result.error) },
    );
  }, [api]);
  ...
  const loadRouting = useCallback(async () => {
    ...
    if (result._tag === "ok") {
      ...
      setRoutingError(describePrismError(result.error));
    }
  }, [api]);
  ...
  const runRestart = async () => {
    ...
    while (step === "poll") {
      await delay(PRISM_RESTART_POLL_MS);
      if (!mounted.current) return;
    ...
    void api.patchAccount(account.id, { disabled: !enabled }).then((result) => {
      ...
      dispatchAccounts(
        result._tag === "ok"
          ...
          : { type: "toggleFailed", id: account.id, error: describePrismError(result.error) },
      );
    });
  };
              ...
              dispatchAccounts(
                result._tag === "ok"
                  ...
                  : {
                      type: "removeFailed",
                      id: account.id,
                      error: describePrismError(result.error),
                    },
              );
            });
          },
...
function useStatusPolling(enabled: boolean, poll: () => Promise<void>) {
  ...
  useEffect(() => {
    if (!enabled || !shouldPollPrismStatus({ focused, appState })) return;
    const interval = setInterval(() => void poll(), PRISM_STATUS_POLL_MS);
    ...
    >
      <View className="min-w-0 flex-1">
        ...
        <Text className="text-sm text-foreground-muted" numberOfLines={1}>
          {describePrismAccount(account, relativeTime)}
        </Text>
        {props.error ? <ErrorText>{props.error}</ErrorText> : null}
      </View>
      <ThemedSwitch
...
function AddAccountSection(props: {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => void tick(), PRISM_LOGIN_POLL_MS);
    return () => {
      cancelled = true;
  ...
  const start = (provider: PrismLoginProvider) => {
    ...
    void api.startLogin(provider).then((result) => {
      if (result._tag === "error") {
        dispatch({ type: "startFailed", error: describePrismError(result.error) });
        return;
      }
  ...
  const submitRedirect = () => {
    ...
    void api.completeLogin(sessionId, redirectUrl).then((result) => {
      if (result._tag === "ok") {
        ...
        dispatch({ type: "redirectFailed", sessionId, error: describePrismError(result.error) });
      }
    });
  };
        ...
        {login._tag === "idle" ? (
          <>
            ...
            <View className="flex-row flex-wrap gap-2">
              {PRISM_LOGIN_PROVIDERS.map((provider) => (
                <Chip
                  key={provider.value}
          ...
          <Text className="text-base text-foreground-muted">
            Starting {labelPrismLoginProvider(login.provider)} sign-in…
          </Text>
        ) : login._tag === "pending" ? (
          <>
            <Text className="text-base text-foreground">
              Finish the {labelPrismLoginProvider(login.provider)} sign-in in your browser.
            </Text>
            {login.userCode ? (
          ...
          <>
            {login._tag === "completed" ? (
              <Text className="text-base text-foreground">
                {labelPrismLoginProvider(login.provider)} account added
...
function RoutingSection(props: {
  ...
  readonly onSelect: (strategy: PrismRoutingStrategy) => void;
}) {
  return (
