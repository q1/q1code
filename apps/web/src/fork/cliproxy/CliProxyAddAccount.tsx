import type { CliProxyLoginProvider } from "@q1code/core/cliproxyApi";
...
import { CopyValueButton, openExternalUrl, reportCliProxyError } from "./cliproxyUi";
import { type CliProxyApi, describeCliProxyCallError } from "./useCliProxyApi";
 ...
 */
export function CliProxyAddAccountSection({
  api,
  writable,
  onCompleted,
}: {
  readonly api: CliProxyApi | null;
  ...
  const cancelLogin = async () => {
    ...
    if (result._tag === "error" && result.error._tag !== "CliProxyNotFoundError") {
      reportCliProxyError("Could not cancel the sign-in", result.error);
    }
  };
  ...
  return (
    <SettingsSection {...searchableSetting("cliproxy-add-account")}>
      <SettingsRow
        title="Sign in"
      ...
      >
        {flow._tag === "pending" ? (
          <div className="mt-2 mb-2 space-y-3 rounded-lg border border-border/60 bg-muted/30 p-3 text-xs">
            ...
            {flow.flow === "redirect" ? (
              <div className="space-y-1">
                <label className="text-muted-foreground" htmlFor="cliproxy-login-redirect-url">
