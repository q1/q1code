...
const makeLayers = (input: {
  ...
  binaryLayer.pipe(
    ...
    Layer.provide(Layer.succeed(CliProxyBundledRoots, [input.bundledRoot])),
...
it.layer(NodeServices.layer)("CliProxyBinary", (it) => {
  it.effect("downloads, verifies, extracts, then serves the cache without downloading again", () =>
    Effect.gen(function* () {
      ...
      const downloader = makeDownloader(
        new Map([
          [
            cliproxyChecksumsUrl(VERSION),
            new TextEncoder().encode(`${sha256Hex(archive)}  ${ASSET}\n`),
          ],
          [cliproxyReleaseUrl(PLATFORM, ARCHITECTURE, VERSION)!, archive],
        ]),
      );
      ...
      assert.equal(
        first.path,
        path.join(root, "base", "cliproxy", "bin", VERSION, "cli-proxy-api"),
      );
      assert.equal((yield* fs.stat(first.path)).mode & 0o111, 0o111);
      assert.equal(yield* fs.readFileString(first.path), "#!/bin/sh\necho fake\n");
      assert.deepEqual(yield* fs.readDirectory(path.join(root, "base", "cliproxy", "bin")), [
        VERSION,
      ]);
    ...
    Effect.gen(function* () {
      ...
      const downloader = makeDownloader(
        new Map([
          [
            cliproxyChecksumsUrl(VERSION),
            new TextEncoder().encode(`${"0".repeat(64)}  ${ASSET}\n`),
          ],
          [cliproxyReleaseUrl(PLATFORM, ARCHITECTURE, VERSION)!, archive],
        ]),
      );
      ...
      assert.isFalse(yield* fs.exists(path.join(root, "base", "cliproxy", "bin", VERSION)));
    }).pipe(Effect.scoped),
  );
