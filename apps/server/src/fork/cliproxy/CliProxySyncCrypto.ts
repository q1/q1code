...
const TAG_BYTES = 16;
const KEY_BYTES = 32;
const HKDF_SALT = "q1code-cliproxy-sync";
const HKDF_INFO = "auth-file-v1";

/** An opaque 32-byte AES key. Keep it out of logs and error messages. */
export type SyncKey = Uint8Array & { readonly __brand: "CliProxySyncKey" };

export class CliProxySyncCryptoError extends Schema.TaggedErrorClass<CliProxySyncCryptoError>()(
  "CliProxySyncCryptoError",
  {
...
): Effect.Effect<string, CliProxySyncCryptoError> =>
  Effect.try({
    try: () => {
    ...
    catch: () => new CliProxySyncCryptoError({ operation: "encrypt" }),
  });

export const decryptSyncEntry = (
  key: SyncKey,
  ciphertext: string,
): Effect.Effect<Uint8Array, CliProxySyncCryptoError> =>
  Effect.try({
    try: () => {
    ...
    catch: () => new CliProxySyncCryptoError({ operation: "decrypt" }),
  });
