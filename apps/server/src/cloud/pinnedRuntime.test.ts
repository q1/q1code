...
import { forkPinnedRuntimeTestLayer } from "../fork/releaseTarball.testing.ts"; // fork: base
...
it.layer(forkPinnedRuntimeTestLayer)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("installs through pnpm when its Node runtime has no npm executable", () =>
    Effect.gen(function* () {
      ...
      assert.equal(path.basename(commands[1]!.args.at(-1)!), releaseTarballName("1.2.3"));
