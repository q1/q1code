...
- **status**: active (phase 1: sidecar lifecycle and provider wiring; phase 2: accounts HTTP API, client runtime, cross-machine sync. Next phase: the Accounts UI)
...
- **later phases**: Accounts panel in Settings (web and mobile over `@t3tools/client-runtime/fork`), provider-instance registration from the UI, sync over `relay-selfhost`, sync deletions, persisting a routing-strategy change back into `fork.json` (today a `PUT routing` lasts until the next sidecar start).
...
- **owned dirs**: `apps/server/src/fork/cliproxy/`, `packages/fork-core/src/cliproxy*`, `packages/client-runtime/src/fork/cliproxyClient*`, the sidecar step in `.github/workflows/fork-release.yml`, later `apps/web/src/fork/accounts/` and `apps/mobile/src/fork/accounts/`. Runtime state under `~/.q1code/cliproxy/` (`config.yaml`, `auths/`, `bin/`, `codex-home/`) and secrets `cliproxy-api-key`, `cliproxy-management-secret` in the server secret store.
- **seams**:
