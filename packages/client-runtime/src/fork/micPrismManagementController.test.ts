...
import {
  ...
  type MicPrismManagementRunner,
} from "./micPrismManagementController.ts";
...
const testRunner = Effect.gen(function* () {
  ...
  yield* Effect.forever(
    Effect.gen(function* () {
      ...
      const cancelled = Effect.callback<never, MicIdentityUnauthorizedError>((resume) => {
        const abort = () =>
          resume(Effect.fail(new MicIdentityUnauthorizedError({ reason: "revoked-session" })));
  ...
  const run: MicPrismManagementRunner = (effect, signal) =>
    new Promise((resolve, reject) => {
      Queue.offerUnsafe(jobs, {
        effect,
        signal,
        resolve: (value) => resolve(value as Effect.Success<typeof effect>),
        reject,
      });
    });
  return run;
...
function fixture() {
  ...
  const http = remoteHttpClientLayer((async (url, init) => {
    ...
    if (path === "/v1/prism/discovery")
      return Response.json({
        ...
        service: {
          ...binding,
          displayName: "PC",
          apiOrigin: "https://pc.example.test",
          inferenceOrigin: "https://pc.example.test",
          status: "paired",
          protocolVersion: 1,
    ...
    if (init?.method && init.method !== "GET") {
      writes.push(init);
      return new Response("private failure", { status: status === 200 ? 409 : status });
    }
    if (nextRead) {
      const read = nextRead;
      nextRead = undefined;
    ...
    if (path === "/prism/v1/accounts")
      return Response.json({ ...binding, settingsRevision: revision, accounts: [account] });
...
describe("Prism management view lifecycle", () => {
  it.effect("does not queue or retry a conflicting edit", () =>
    Effect.gen(function* () {
      const h = fixture();
      yield* Effect.gen(function* () {
        const run = yield* testRunner;
        const controller = createMicPrismManagementController({
          input: h.input,
          permissions: grants,
          run,
        });
  ...
  it.effect("discards an account response that arrives after permission revocation", () =>
    Effect.gen(function* () {
      const h = fixture();
      yield* Effect.gen(function* () {
        const run = yield* testRunner;
  ...
  it.effect(
    "retains last known state while offline and disables writes until an explicit fresh read",
    () =>
      Effect.gen(function* () {
        const h = fixture();
        yield* Effect.gen(function* () {
  ...
  it.effect("drops cached account details when read permission is removed", () =>
    Effect.gen(function* () {
      const h = fixture();
      yield* Effect.gen(function* () {
        const run = yield* testRunner;
  ...
  it.effect("can reactivate after disposal without retaining a stuck loading state", () =>
    Effect.gen(function* () {
      const h = fixture();
      yield* Effect.gen(function* () {
        const run = yield* testRunner;
