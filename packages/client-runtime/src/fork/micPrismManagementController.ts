import type {
  MicPrismAccountPatch, MicPrismAccountsState, MicPrismAvailability, MicPrismLoginProvider,
  MicPrismLoginStarted, MicPrismManagedLoginStatus, MicPrismSettings, MicPrismSettingsState,
...
import {
  cancelMicPrismLogin, completeMicPrismLogin, deleteMicPrismAccount, getMicPrismAccounts,
  getMicPrismAvailability, getMicPrismLoginStatus, getMicPrismSettings, patchMicPrismAccount,
  setMicPrismSettings, startMicPrismLogin,
...
export function createMicPrismManagementController(options: {
  ...
  let state: MicPrismManagementState = { accounts: null, settings: null, availability: null, login: null, loginStatus: null,
    loading: false, busy: false, error: null, notice: null, receivedAt: 0 };
  let active = false;
  let sequence = 0;
  ...
  const message = (error: unknown) => typeof error === "object" && error !== null && "message" in error && typeof error.message === "string"
    ? error.message : "Prism could not be reached. Refresh to check its current state.";
  ...
  const refresh = async (afterWrite = false) => {
    ...
    emit({ loading: true });
    try {
      const values = await options.run(Effect.all({
        accounts: allowed("prism:accounts:read") ? getMicPrismAccounts(input).pipe(Effect.result) : Effect.succeed(null),
        settings: allowed("prism:settings:read") ? getMicPrismSettings(input).pipe(Effect.result) : Effect.succeed(null),
        availability: allowed("prism:inference") ? getMicPrismAvailability(input).pipe(Effect.result) : Effect.succeed(null),
        loginStatus: state.login && allowed("prism:accounts:write")
          ? getMicPrismLoginStatus({ ...input, sessionId: state.login.sessionId }).pipe(Effect.result)
          : Effect.succeed(null),
      }, { concurrency: "unbounded" }), controller.signal);
      if (!valid(ticket)) { if (active && ticket === sequence) emit({ loading: false, busy: false }); return; }
      ...
      emit({
        ...
        availability: values.availability?._tag === "Success" ? values.availability.success : state.availability,
        loginStatus: values.loginStatus?._tag === "Success" ? values.loginStatus.success : state.loginStatus,
  ...
  const write = async <A>(permission: MicPrismPermission,
    effect: Effect.Effect<A, MicPrismManagementClientError, HttpClient.HttpClient | Crypto.Crypto>,
    accept: (value: A) => Partial<MicPrismManagementState> = () => ({ notice: "Confirmed by Prism." }),
  ) => {
    if (!isCurrent() || !allowed(permission) || state.busy || state.loading || state.error) return;
    const ticket = ++sequence;
    controller?.abort();
    controller = new AbortController();
  ...
  const loginMutation = (action: "callback" | "cancel", redirectUrl = "") => {
    ...
    return write("prism:accounts:write", Effect.gen(function* () {
    ...
    }), (loginStatus) => ({ loginStatus, notice: loginStatus.status === "completed"
      ? "Sign-in saved. Check account health and model availability before using it."
      : loginStatus.status === "cancelled" ? "Sign-in cancelled." : "Callback sent. Waiting for the provider." }));
  };
  ...
  return {
    ...
    dispose: () => { active = false; sequence++; controller?.abort(); state = { ...state, loading: false, busy: false }; },
    updatePermissions: (next: ReadonlyArray<string>) => {
      if (permissions.length === next.length && permissions.every((value) => next.includes(value))) return;
      permissions = next;
      sequence++;
      controller?.abort();
      emit({ loading: false, busy: false,
    ...
    startLogin: (provider: MicPrismLoginProvider, revision: string) =>
      write("prism:accounts:write", startMicPrismLogin({ ...mutation(revision), provider }),
    ...
    clearLogin: () => { if (!state.busy) emit({ login: null, loginStatus: null, notice: null }); },
  };
}
